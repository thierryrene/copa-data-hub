...
// chamam estes helpers quando isMockActive() retorna true.

import { getMockScenario } from './mockMode.js';
...
function buildLineupForApi(squad, teamApiId, side) {
...
function buildEvents(scenario, homeApiId, awayApiId, homeName, awayName, homeSquad, awaySquad) {
  ...
  // Pênalti home aos 58' (live=2x1, finished=3x1)
  e.push({
...
function buildPlayersBlock(scenario, homeApiId, awayApiId, homeName, awayName, homeSquad, awaySquad) {
  ...
  const ratingFor = (idx) => {
    // Ratings de 6.4 a 9.1 distribuídos
...
export function mockMatchData(fixture) {
...
// Provável XI mesmo em pré-jogo (para o cenário pre/injuries-heavy mostrar formação)
...
export function mockTeamForm(teamApiId, teamCode) {
  ...
  // Rotaciona o array conforme o seed para variar entre os times
  const offset = seed % 5;
...
export function mockInjuries(fixture) {
  const scenario = getMockScenario();
  if (!scenario) return [];
  const homeApiId = getTeamApiId(fixture.home) || 9001;
  const awayApiId = getTeamApiId(fixture.away) || 9002;
  ...
  // outros cenários: 1 desfalque por time
  return [
...
// ── Times fictícios para campeonatos externos (UCL, Brasileirão, etc.) ──
const LEAGUE_TEAMS = {
