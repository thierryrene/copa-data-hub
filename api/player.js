...
import { mockPlayerDetails } from '~/utils/mockData';
import { slugify } from '~/utils/slug';
import { fetchWikipediaPlayerSummary } from '~/api/wikipedia';
...
// Indice slug → id mantido em sessionStorage para deep-links resolverem.
...
export async function resolvePlayerIdBySlug(slug) {
  ...
  // 1) índice direto em sessionStorage
  const cached = getRegisteredSlugId(slug);
  if (cached) return cached;

  // 2) busca reversa em players cacheados (client-only)
  ...
  // 3) busca na API-Football por nome
...
export async function fetchPlayerDetails(playerId) {
  ...
  if (mock.isActive) return mockPlayerDetails(playerId);
